...
def get_ai_analysis(metrics_data, api_key):
    ...
    try:
        ...
        - Biến động: Thấp là ổn định, cao là rủi ro
        
        Hãy viết nhận xét bằng tiếng Việt, rõ ràng và dễ hiểu.
        """
        ...
        if "API_KEY" in error_msg or "api_key" in error_msg.lower():
            return f"❌ Lỗi: API Key không hợp lệ. Vui lòng kiểm tra lại API key của bạn."
        elif "quota" in error_msg.lower() or "limit" in error_msg.lower():
            return f"❌ Lỗi: Đã vượt quá giới hạn sử dụng API. Vui lòng thử lại sau."
        else:
            return f"❌ Đã xảy ra lỗi: {error_msg}"
      
# Treemap Visualization
# Thay thế hàm plot_final_allocation_treemap cũ bằng hàm mới này
def calculate_allocation_for_date(aligned_states, stock_dim, tickers, date_index):
    """Tính phân bổ danh mục cho một ngày cụ thể"""
    try:
        states_at_date = [run_states[date_index] for run_states in aligned_states]
        allocations = []
        
        for state in states_at_date:
            cash = state[0]
            prices = np.array(state[1:1 + stock_dim])
            holdings = np.array(state[1 + stock_dim:1 + 2 * stock_dim])
        ...
        final_allocations = np.mean(allocations, axis=0) * 100
        ...
        return pd.DataFrame({f"Day {date_index}": final_allocations}, index=tickers).T
    except Exception as e:
        st.error(f"Lỗi khi tính phân bổ: {e}")
        return None

def plot_allocation_treemap(df_allocations, model_name, selected_date):
    """Vẽ treemap phân bổ danh mục"""
    try:
        ...
        fig.update_layout(
            title=f"Phân bổ Danh mục tại {selected_date} - Model: {model_name}",
            width=1000,
            height=800,
...
def run_backtest(start_date, end_date):
    ...
    # Lưu thêm thông tin để tính phân bổ theo ngày
    return average_assets, current_dates, metrics, df_allocations, aligned_states, stock_dim, tickers
# Config
st.set_page_config(
...
if st.session_state.page == "Main":
        ...
        if end_date > today:
            ...
            if st.button("CHẠY KIỂM THỬ", use_container_width=True):
                with st.spinner("Đang chạy backtest..."):
                    ...
                    assets, dates, metrics, df_allocations, aligned_states, stock_dim, tickers = run_backtest(start_date, end_date)
                    if assets is not None:
                        st.session_state.assets = assets
                        st.session_state.dates = dates
                        st.session_state.metrics = metrics
                        st.session_state.df_allocations = df_allocations
                        st.session_state.aligned_states = aligned_states
                        st.session_state.stock_dim = stock_dim
    ...
    if "backtest_run" in st.session_state and st.session_state.backtest_run:
        ...
        st.table(df_metrics)
        # --- Thêm phần phân tích AI ---
        st.write("---")
        st.subheader("🤖 Nhận xét Tình hình Tài chính (AI)")
        ...
        # Thêm bộ chọn ngày
        st.write("---")
        st.subheader(":red[Phân bổ Danh mục theo Ngày]")

        # Lấy danh sách ngày có sẵn
        available_dates = [d.date() for d in st.session_state.dates]
        ...
        with col_date1:
            selected_date_input = st.date_input(
                ":red[Chọn ngày để xem phân bổ danh mục:]",
                value=max_date,  # Mặc định là ngày cuối
                min_value=min_date,
                max_value=max_date
            )

        # Tìm index của ngày gần nhất
        with col_date2:
            # Tìm ngày gần nhất trong danh sách
            selected_date_input_dt = pd.to_datetime(selected_date_input)
            dates_series = pd.Series(st.session_state.dates)
            idx = (dates_series - selected_date_input_dt).abs().argmin()
            actual_selected_date = st.session_state.dates[idx]
            
            st.metric("Ngày thực tế được chọn", actual_selected_date.strftime("%Y-%m-%d"))
            if selected_date_input != actual_selected_date.date():
                st.info(f"📅 Ngày gần nhất có dữ liệu: {actual_selected_date.strftime('%Y-%m-%d')}")

        # Tính và hiển thị phân bổ cho ngày được chọn
        df_alloc_selected = calculate_allocation_for_date(
            st.session_state.aligned_states,
            st.session_state.stock_dim,
            st.session_state.tickers,
            idx
        )

        if df_alloc_selected is not None:
            st.write(f"**Phân bổ danh mục tại ngày {actual_selected_date.strftime('%Y-%m-%d')}:**")
            st.dataframe(df_alloc_selected)
            plot_allocation_treemap(df_alloc_selected, "DAPO (Cvar 0.01 Phobert 1a.3b)", actual_selected_date.strftime('%Y-%m-%d'))
